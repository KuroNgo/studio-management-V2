<template>
  <main class=" py-20 h-screen">
    ...
    <!-- alert thông báo -->
    <div id="toastBox"
      class="pt-9 w-1/4 flex overflow-hidden flex-col-reverse gap-y-6 fixed top-0 right-0 show-alert transform transition-transform duration-1000 ease-in-out">
    </div>
...
export default {
  ...
  methods: {
    ...
    async Run() {
      this.CheckForm();
      if (this.inputEmail.error === false &&
        this.inputUserName.error === false &&
        this.inputPassword.error === false &&
        this.inputPhoneNumber.error === false &&
        this.enterPassword.error === false) {
    ...
    async RegisterS() {
      ...
      await useUserStore().Register(this.inputFullName.value, this.inputUserName.value, this.inputEmail.value, this.inputPassword.value, this.inputPhoneNumber.value, this.role, this.provider, this.verified, this.enable)
